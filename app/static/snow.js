...
window.snow = (function() {
  ...
  function Fall(options) {
    ...
    this.stickingRatio = options.stickingRatio || 0.4;
    this.maxHeightRatio = options.maxHeightRatio || 0.25;
    this.dying = false;
    for (var i = 0; i < this.flakes.length; i++)
      this.flakes[i] = new Flake(this);
  }
    ...
    if (n % 2) {
      for (var i = 1; i < W; i++) {
        ...
        if (d > 1) {
          heights[i] -= 0.7 * d;
          heights[i - 1] += 0.3 * d;
          if (i > 1) heights[i - 2] += 0.2 * d;
          if (i > 2) heights[i - 3] += 0.1 * d;
        }
      }
    } else {
      for (var i = 0; i < W - 1; i++) {
        ...
        if (d > 1) {
          heights[i] -= 0.7 * d;
          heights[i + 1] += 0.3 * d;
          if (i < W - 2) heights[i + 2] += 0.2 * d;
          if (i < W - 3) heights[i + 3] += 0.1 * d;
        }
      }
    }
    n++;
  ...
  function Flake(fall) {
    ...
    if (fall.maxRadius > 1) {
      this.speed =
        fall.minSpeed +
        ((fall.maxSpeed - fall.minSpeed) * (this.radius - 1)) /
          (fall.maxRadius - 1);
    } else {
      this.speed = rnd(fall.minSpeed, fall.maxSpeed);
    }
    this.omega = rnd(0.02, 0.13);
  }
...
window.onload = function() {
  snow.start({ flakeCount: 300, maxRadius: 5, minSpeed: 0.2, maxSpeed: 3 });
};
